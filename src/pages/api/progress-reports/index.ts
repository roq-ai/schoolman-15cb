...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getProgressReports() {
    ...
    const response = await prisma.progress_report
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'progress_report'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createProgressReport() {
    ...
    const data = await prisma.progress_report.create({
      data: body,
    });

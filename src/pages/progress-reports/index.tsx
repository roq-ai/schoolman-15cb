...
type ColumnType = ColumnDef<ProgressReportInterface, unknown>;

interface ProgressReportListPageProps {
...
export function ProgressReportListPage(props: ProgressReportListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ProgressReportInterface>>(
    () => `/progress-reports?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ProgressReportInterface) => {
    if (hasAccess('progress_report', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/progress-reports/view/${row.id}`);
    }
  };
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="progress_report" />
      <Flex justifyContent="space-between" mb={4}>
        ...
        {hasAccess('progress_report', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/progress-reports/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
